...
int tlum_na_ang(char n[1000], char m[1000]){
	int dl,dl2,i;
	dl=0;
	dl2=0;
	int flag=0;
...
void tlumacz_na_ang(){
	...
	printf(" Wybrales tlumaczenie z jezyka polskiego na jezyk angielski! \n");
	printf(" Podaj slowko do przetlumaczenia: ");
	printf("\x1b[37m\x1b[40m\n");
	scanf("%s", tab2);
	FILE *fp;
    if ((fp=fopen("wszystkonaang.txt", "r"))==NULL)
    {
    ...
    while(fscanf(fp, "%d", &i) != EOF)
    {
	 ...
	 bool=tlum_na_ang(tab2,tab[i]);
	 if(bool==1){
		 printf("\x1b[37m\x1b[44m\n");
		 printf(" %s w jezyku angielskim to: %s\n",tab2,tab[i-1]);
		 printf("\x1b[37m\x1b[40m\n");
		 flag++;
		 break;
	 };
